    ...
    public partial class PowerSpectrumChartView : ChartViewBase
    {
        ...
        protected override void UpdateChart(object args)
        {
            try
            {
                    ...
                    case SignalPreProccessType.None:
                        {
                            ...
                            {
                                BaseWaveSignal signal = (BaseWaveSignal)ViewModel.Signal;
                                if (signal.FilterFFTLength == 0 || signal.FilterFrequency == null || signal.FilterPowerSpectrum == null)
                                {
                                    return;
                                }
                                ...
                                for (int i = 0; i < signal.FilterFFTLength; i++)
                                {
                                    ...
                                    series.Points[i].Y = signal.FilterPowerSpectrum[i];
                                }
                                ...
                                var fftValuesDict = signal.FilterPowerSpectrum.Select((s, i) => new { Key = i, Value = s }).OrderByDescending(o => o.Value).Take(6);
                                foreach (var item in fftValuesDict)
                                {
                ...
                {
                    BaseWaveSignal signal = (BaseWaveSignal)ViewModel.Signal;
                    if (signal.ProcessingFFTLength(processName) == 0 || signal.FrequencyList == null || signal.PowerSpectrumList == null 
                        || !signal.FrequencyList.ContainsKey(processName) || !signal.PowerSpectrumList.ContainsKey(processName))
                    {
                        return;
                    }
                    ...
                    for (int i = 0; i < signal.ProcessingFFTLength(processName); i++)
                    {
                        ...
                        series.Points[i].Y = signal.PowerSpectrumList[processName][i];
                    }
                    ...
                    var fftValuesDict = signal.PowerSpectrumList[processName].Select((s, i) => new { Key = i, Value = s }).OrderByDescending(o => o.Value).Take(6);
                    foreach (var item in fftValuesDict)
                    {
        ...
        private void CreateChart()
        {
            ...
            {
                EventAggregatorService.Instance.EventAggregator.GetEvent<ThrowExceptionEvent>().Publish(Tuple.Create<string, Exception>("在线监测-功率谱-初始化", ex));
                if (m_chart != null)
                {
                    m_chart.EndUpdate();
                }
