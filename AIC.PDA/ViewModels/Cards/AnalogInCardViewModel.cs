...
{
    public class AnalogInCardViewModel : BindableBase, INavigationAware, IConfirmNavigationRequest
    {
        private readonly IPDAService _pdaService;
        public AnalogInCardViewModel(IPDAService pdaService)
        {
        ...
        private AnalogInCardModelWrapper card;
        public AnalogInCardModelWrapper Card
        {
            get { return card; }
            set
            {
        ...
        public void OnNavigatedTo(NavigationContext navigationContext)
        {
            ...
            var model = navigationContext.Parameters["Card"] as AnalogInCardModel;
            if (model != null)
            {
                if (Card == null || Card.Model != model)
                {
                    Card = new AnalogInCardModelWrapper(model);
                }
            }
            else
            {
