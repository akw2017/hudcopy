...
{
    public class LowPassFilter : NotifyDataErrorInfoBase
    {
        ...
        private readonly LowPassFilterValidator validator;
        private bool isRPMBinding;


        public LowPassFilter(double sampleFre = 1024)
        {
            _originalValues = new Dictionary<string, object>();
            validator = new LowPassFilterValidator();
        ...
        {
            PassbandFre = (sampleFre / 2.56) / 3;
            StopbandFre = (sampleFre / 2.56) * 2 / 3;
            Save(null);
        }
        ...
        {
            PassbandAttenuationDB = (double)_originalValues["PassbandAttenuationDB"];
            StopbandAttenuationDB = (double)_originalValues["StopbandAttenuationDB"];
            StopbandFre = (double)_originalValues["StopbandFre"];
            PassbandFre = (double)_originalValues["PassbandFre"];
            IsChanged = false;
        }
        ...
        public double[] Filter(double[] input, int samplePoint, double sampleFre, double rpm = 0)
        {
            if (!IsRPMBinding)
            {
                if ((double)_originalValues["StopbandFre"] > sampleFre / 2.56)
                {
                    throw new Exception(string.Format("阻带频率必须小于等于采样频率的1/2.56({0})", sampleFre / 2.56));
                }
                return Algorithm.Instance.LowPassFilter(input, samplePoint, sampleFre,
                    (double)_originalValues["PassbandFre"],
                    (double)_originalValues["StopbandFre"],
                    (double)_originalValues["PassbandAttenuationDB"],
                    (double)_originalValues["StopbandAttenuationDB"]);
            }
            else
            {
                if ((double)_originalValues["StopbandFre"] * (rpm / 60) > sampleFre / 2.56)
                {
                    throw new Exception(string.Format("阻带频率必须小于等于采样频率的1/2.56({0})", sampleFre / 2.56));
                }
                return Algorithm.Instance.LowPassFilter(input, samplePoint, sampleFre,
                    (double)_originalValues["PassbandFre"] * (rpm / 60),
                    (double)_originalValues["StopbandFre"] * (rpm / 60),
                    (double)_originalValues["PassbandAttenuationDB"],
                    (double)_originalValues["StopbandAttenuationDB"]);
            }
        }
