...
namespace AIC.Core.DiagnosticModels
{
    public class DiagnosticInfoClass
    {
        ...
        private static double[] DecToBinArray(int dec)
        {
            string bin = System.Convert.ToString(dec, 2);
            bin = bin.PadLeft(6, '0');
            double[] binArray = bin.ToArray().Select(p => Convert.ToDouble(p.ToString())).Reverse().ToArray();
            return binArray;
        }

        public static string BinArrayToString(double[] binArray)
        {
            string bin = string.Join("",binArray.Select(p => Convert.ToInt32(p)).Reverse());
            switch(bin.PadLeft(6, '0'))
            {
                case "000001": return "不平衡";
                case "000010": return "不对中";
                case "000011": return "松动";
                case "000100": return "频谱上有占能量很大比率的频率";
                case "000101": return "没有找到合适的故障原因";
                default: return null;
            }
        }
        ...
        public static bool GetEnergy(double[] Frequency, double[] Amplitude, double frequency, double frequencyInterval, int number, out double energy, out double m)
        {
            ...
            {
                energysum += Math.Pow(Amplitude[key.Index], 2);
            }
            energy = Math.Sqrt(energysum / 2);
            List<double> mlist = new List<double>();
            foreach (var key in keys)
            {
                mlist.Add(Amplitude[key.Index]);
            }
            if (mlist.Count > 0)
            {
                m = mlist.Max();
