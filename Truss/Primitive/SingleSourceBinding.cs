	...
	public abstract class SingleSourceBinding : BindingBase
	{
		...
		private void OnActivatedCore()
		{
			...
			if (SourcePropertyExpression != null && (Mode == BindingMode.TwoWay || Mode == BindingMode.OneWayToTarget))
			{
				...
				SourcePropertyExpression.ValueChanged += _sourcePropertyExpression_ValueChanged;
		...
		private void _sourcePropertyExpression_ValueChanged(object sender, EventArgs e)
		{
			PushSourceToTarget();
		}
		...
		private void PushSourceToTargetCore(object state)
		{
			...
			if (_converter != null)
			{
				newValue = _converter.ConvertSourceToTarget(newValue, TargetPropertyExpression.PropertyType, _converterParameter);
			}
		...
		private void PushTargetToSourceCore(object state)
		{
			...
			if (_converter != null)
			{
				newValue = _converter.ConvertTargetToSource(newValue, SourcePropertyExpression.PropertyType, _converterParameter);
			}
