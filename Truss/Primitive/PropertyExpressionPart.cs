...
using Kent.Boogaart.Truss.Diagnostics;
...
namespace Kent.Boogaart.Truss.Primitive
{
	...
	public abstract class PropertyExpressionPart
	{
		...
		private static readonly ICollection<IPropertyObservationStrategy> _propertyObservationStrategies = new List<IPropertyObservationStrategy>(new IPropertyObservationStrategy[] { new INPCPropertyObservationStrategy(), new XxxChangedPropertyObservationStrategy(), new DependencyPropertyPropertyObservationStrategy() });
		...
		public object Value
		{
			...
			set
			{
				...
				if (!object.Equals(oldValue, value))
				{
					...
					{
						Log.Write(TraceEventType.Error, "An error occurred attempting to set property with name '{0}': {1}", PropertyName, ex);
					}
					finally
					{
		...
		{
			Log.Write(TraceEventType.Information, "Attempting automatic conversion of value from type {0} to type {1} for property '{2}'.", value.GetType(), PropertyType, PropertyName);
			...
			{
				Log.Write(TraceEventType.Error, "Conversion Failed. Unable to convert type {0} to {1} for property '{2}' due to an exception: {3}.", value.GetType(), PropertyType, PropertyName, ex);
			}
