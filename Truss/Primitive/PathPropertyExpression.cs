...
namespace Kent.Boogaart.Truss.Primitive
{
	...
	public sealed class PathPropertyExpression : PropertyExpression
	{
		...
		/// </summary>
		/// <param name="obj">
		/// The object against which the path will be resolved.
		/// </param>
		/// <param name="path">
		/// The path.
		/// </param>
		...
		public static PathPropertyExpression FromPath(object obj, string path)
		{
			...
			{
				var part = new PathPropertyExpressionPart(nextPart, pathParts[i]);
				nextPart = part;
		...
		private sealed class PathPropertyExpressionPart : PropertyExpressionPart
		{
			...
			public PathPropertyExpressionPart(PathPropertyExpressionPart nextPart, string pathPart)
				: base(nextPart)
			{
			...
			protected override object GetValueCore()
			{
				...
				{
					return PropertyExpressionPart.Unresolved;
				}
			...
			protected override void OnValueChanged()
			{
				...
				else
				{
					...
					{
						Log.Write(TraceEventType.Warning, "Binding failure: unable to resolve property '{0}' on type '{1}'.", _pathPart, obj.GetType().FullName);
					}
				}
