...
namespace Kent.Boogaart.Truss.Primitive
{
	...
	{
        private static readonly ExceptionHelper _exceptionHelper = new ExceptionHelper(typeof(PropertyExpression));
		...
		protected PropertyExpression(object obj, IEnumerable<PropertyExpressionPart> parts)
		{
			...
			parts.AssertNotNull("parts", true);
			_parts = new List<PropertyExpressionPart>(parts);

			_exceptionHelper.ResolveAndThrowIf(_parts.Count == 0, "OnePartRequired", "parts");
